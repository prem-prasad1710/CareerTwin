...
export async function predict(userId: string): Promise<{ primary: TimelineEntry[]; alternate: TimelineEntry[] }> {
  ...
  const roleProgression = getRoleProgression(currentRole, years);
  ...
  for (const entry of [...primary, ...alternate]) {
    await prisma.careerTimeline.upsert({
      where: { id: `${userId}-${entry.year}-${entry.scenario}` },
      update: entry as any,
      create: { userId, ...entry } as any,
    }).catch(() => {
      prisma.careerTimeline.create({ data: { userId, ...entry } as any });
    });
  }
...
function getRoleProgression(currentRole: string, years: number) {
