...
export async function compute(userId: string): Promise<CareerDna & { strengths: string[]; weaknesses: string[] }> {
  ...
  const dna = calculateDna(user);
  const entries = Object.entries(dna) as [string, number][];
  const sorted = [...entries].sort((a, b) => b[1] - a[1]);

  const strengths = sorted.slice(0, 3).map(([k, v]) => `${formatDimension(k)}: ${v}`);
  const weaknesses = sorted.slice(-3).map(([k, v]) => `${formatDimension(k)}: ${v}`);
...
function calculateDna(user: any): CareerDna {
